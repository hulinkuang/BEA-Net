...
if __name__ == "__main__":
    dataset_dir = Path("/home/kuanghl/Dataset/JSRT/")
    # data_path = dataset_dir / "Original"
    # masks_path = dataset_dir / "Ground Truth"

    output_folder = Path("/home/kuanghl/Codes/CoTr_KSR/nnUNet/nnU_data/nnUNet_raw_data_base/nnUNet_raw_data/Task311_JSRT")
    ...
    for img_path in dataset_dir.glob('*.IMG'):
        all_ids.append(img_path.stem)
        arr = np.fromfile(img_path, dtype=">i2").reshape((2048, 2048))
        # arr = np.array(img)
        arr = zoom(arr, (256 / arr.shape[0], 256 / arr.shape[1]), order=3)
        convert_2d_image_to_nifti(arr, (img_dir / img_path.stem).as_posix(), is_seg=False)

    for seg_path in dataset_dir.glob('*.gif'):
    ...
    train, test = train_test_split(all_ids, test_size=0.2, shuffle=True, random_state=99)
