# Coding by LiuN
# Datatime:2021/3/29 18:53
# File name:edg_launet.py
# Tool by: PyCharm
...
class MyLayer(Layer):
    ...
    def call(self, x):
        ...
        seg_edg = K.tf.layers.Conv2D(in_shape[3], 1, padding='same')(seg_edg)
...
def joint_loss(y_true, y_pred):
    ...
    lossseg = w1 * generalized_dice_loss(y_true, y_pred) + w2 * cross_entropy_balanced(y_true, y_pred)
...
def SqueezeBodyEdge(x, filter, H, W):
    ...
    down1 = Conv2D(filters=filter, kernel_size=3, strides=2, kernel_initializer='he_normal')(x)
    down1 = BatchNormalization(axis=3)(down1)
    down1 = Activation('relu')(down1)

    seg_down = Lambda(Interp, arguments={'shape': (np.int32(H), np.int32(W))})(down)
    seg_down1 = Lambda(Interp, arguments={'shape': (np.int32(H), np.int32(W))})(down1)
    flow = concatenate([x, seg_down, seg_down1], axis=3)
...
def unet(pretrained_weights=None, input_size=(256, 256, 1)):
    ...
    # edg
    edg0 = Conv2D(1, 1)(edge3)
    edg = Activation('sigmoid', name='edg')(edg0)

    # final_seg0 = concatenate([conv9, edg], axis=3)
    final_seg0 = Add()([conv9, edg])
    final_seg0 = Conv2D(3, 1)(final_seg0)
    final_seg = Conv2D(1, 1, activation='sigmoid', name='final_seg')(final_seg0)

    model = Model(input=inputs, output=[final_seg, edg, conv9, final_seg0])
    ...
    model.compile(optimizer=Adam(lr=1e-4), loss={'final_seg': joint_loss, 'edg': 'binary_crossentropy'},
                  ...
                  metrics={'final_seg': dice_coef, 'edg': 'accuracy'})
